...
-------------------------------------------
  ...
  # ============================================
# STEP 2 - Detect & Handle Missing Value
# ============================================
...
# Count missing values per column (before fixing)
...
# Convert character columns to numeric then fill with mean
...
data$wildfire_incidents[is.na(data$wildfire_incidents)] <-
  mean(data$wildfire_incidents, na.rm = TRUE)

data$air_quality_index[is.na(data$air_quality_index)] <-
  mean(data$air_quality_index, na.rm = TRUE)

data$climate_risk_score[is.na(data$climate_risk_score)] <-
  mean(data$climate_risk_score, na.rm = TRUE)
...
# Visualize outliers in wildfire_incidents
...
data_clean <- data[
  data$wildfire_incidents >= (Q1 - 1.5 * IQR_val) &
    data$wildfire_incidents <= (Q3 + 1.5 * IQR_val), ]

# Compare before and after
nrow(data)        # before
nrow(data_clean)  # after
...
# Check flood_risk for invalid values
table(data_clean$flood_risk)

# Fix invalid value "56" in flood_risk
get_mode <- function(x) {
  ux <- na.omit(unique(x))
  ux[which.max(tabulate(match(x, ux)))]
}

# Convert back to character to fix
data_clean$flood_risk <- as.character(data_clean$flood_risk)

# Replace invalid value with mode
data_clean$flood_risk[!data_clean$flood_risk %in% c("High", "Medium", "Low")] <- 
  get_mode(data_clean$flood_risk[data_clean$flood_risk %in% c("High", "Medium", "Low")])
...
# Check results (all values must be between 0 and 1)
...
# Compare before and after
nrow(data_clean)   # before
nrow(data_no_dup)  # after
...
# Check current distribution of flood_risk
table(data_no_dup$flood_risk)

# Filter: Keep only High and Medium flood risk
...
nrow(data_filtered)  # before
nrow(data_valid)     # after
...
# ============================================
# STEP 11 - Descriptive Statistics by Target Class
# ============================================
...
# ============================================
# STEP 12 - Compare Average Values Across 
#           Two Categories
# ============================================

# Compare average values across drought_risk (High vs Low)
...
# ============================================
# STEP 13 - Compare Variability Across 
#           Categories
# ============================================

# SD, Variance, IQR and Range of co2_emissions_mt across regions
